#Plots temperature-binned flowline averaged values of cooling/heating rates evaluated for each cell
import yt
import numpy as np
import pandas as pd
...
for k in range(3):
    ...
    T=df.get('temperature').to_numpy() #Extract the four columns as numpy arrays for use later                                                                                   
    cooling=df.get('cooling_rate').to_numpy()
    heating=df.get('heating_rate').to_numpy()
    n_b=df.get('baryon_number_density').to_numpy()
    cooling_time=[]
    for i in range(len(T)):
        cooling_time.append(k_boltz*T[i]/(n_b[i]*cooling[i]))
        #print(cooling_time[i])
    ...
    binned_h=[]
    binned_time=[]  #Array for cooling time median in each bin
    ...
    for i in range(len(t_bins)-1): #Loop through all temperature bins (more specifically, the lower limit of each bin)
        for j in range(len(T)): #Loop through all T values
            if t_bins[i]<=T[j] and T[j]<=t_bins[i+1]: #Check if given T value is in the bin used for the loop
                ...
                time_stats.append(cooling_time[j])
                counter += 1
        ...
        if counter > 0: #If the bin is non-empty, add center to used centers array, the stats values to the revlevant arrays
            ...
            binned_time.append(np.median(time_stats))
            ...
            t_25.append(np.percentile(time_stats, 25))
            t_75.append(np.percentile(time_stats, 75))
            centers.append(all_centers[i])
        counter = 0 #Reset the counter and the stats arrays
        cooling_stats=[]
        heating_stats=[]
        time_stats=[]
    plt.plot(centers, binned_time, label="Mvir=%.2E [M$_\odot$/h]" % Mvir, color=colors[k], linestyle='dashed')
    plt.fill_between(centers, t_25, t_75, alpha=0.2, color=colors[k])
    plt.xscale('log')
    plt.yscale('log')
    '''
...
plt.savefig("/home/dbrobins/repos/radfieldcooling/plots/cooling_time_median.png")
'''
